...
/// Wire-format magic bytes (allow the decoder to fail fast on garbage input).
const AS_REQ_MAGIC: u8 = 0xA1;
const AS_REP_MAGIC: u8 = 0xA2;
const TGS_REQ_MAGIC: u8 = 0xA3;
const TGS_REP_MAGIC: u8 = 0xA4;
const TICKET_MAGIC: u8 = 0xB1;
const ENC_TICKET_PART_MAGIC: u8 = 0xB2;
const ENC_KDC_REP_PART_MAGIC: u8 = 0xB3;
const PA_ENC_TS_ENC_MAGIC: u8 = 0xB4;
const AUTHENTICATOR_MAGIC: u8 = 0xB5;
...
fn put_str(out: &mut Vec<u8>, s: &str) {
    let b = s.as_bytes();
    let len = u16::try_from(b.len()).expect("string length fits in u16");
    put_u16(out, len);
    out.extend_from_slice(b);
}
fn put_string_list(out: &mut Vec<u8>, v: &[String]) {
    let n = u8::try_from(v.len()).expect("string list length fits in u8");
    out.push(n);
    for s in v {
        put_str(out, s);
    }
}
fn put_etype_list(out: &mut Vec<u8>, v: &[EType]) {
    let n = u8::try_from(v.len()).expect("etype list length fits in u8");
    out.push(n);
    for e in v {
...
fn get_u32(cur: &mut &[u8]) -> Result<u32, DecodeError> {
    if cur.len() < 4 {
...
fn get_u16(cur: &mut &[u8]) -> Result<u16, DecodeError> {
    if cur.len() < 2 {
        return Err(DecodeError::UnexpectedEof);
    }
    let mut buf = [0u8; 2];
    buf.copy_from_slice(&cur[..2]);
    *cur = &cur[2..];
    Ok(u16::from_be_bytes(buf))
}
fn get_u8(cur: &mut &[u8]) -> Result<u8, DecodeError> {
    if cur.is_empty() {
        return Err(DecodeError::UnexpectedEof);
    }
    let b = cur[0];
...
fn get_u64(cur: &mut &[u8]) -> Result<u64, DecodeError> {
    if cur.len() < 8 {
        return Err(DecodeError::UnexpectedEof);
    }
    let mut buf = [0u8; 8];
...
fn get_blob(cur: &mut &[u8]) -> Result<Vec<u8>, DecodeError> {
...
fn get_str(cur: &mut &[u8]) -> Result<String, DecodeError> {
    ...
    let b = &cur[..len];
    *cur = &cur[len..];
    let s = std::str::from_utf8(b).map_err(|_| DecodeError::InvalidUtf8)?;
    Ok(s.to_string())
}
fn get_string_list(cur: &mut &[u8]) -> Result<Vec<String>, DecodeError> {
    let n = get_u8(cur)?;
...
fn get_etype_list(cur: &mut &[u8]) -> Result<Vec<EType>, DecodeError> {
    let n = get_u8(cur)?;
    let mut v = Vec::with_capacity(n as usize);
    for _ in 0..n {
        let raw = get_u32(cur)?;
        let e = EType::from_u32(raw).ok_or(DecodeError::UnknownEtype(raw))?;
        v.push(e);
    }
    Ok(v)
}
...
pub fn encode_pa_enc_ts_enc(p: &PaEncTsEnc) -> Vec<u8> {
    ...
    out.push(PA_ENC_TS_ENC_MAGIC);
    put_u64(&mut out, p.patimestamp as u64);
    put_u32(&mut out, p.pausec);
    out
}

pub fn decode_pa_enc_ts_enc(b: &[u8]) -> Result<PaEncTsEnc, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != PA_ENC_TS_ENC_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: PA_ENC_TS_ENC_MAGIC,
            actual: magic,
        });
    }
    let patimestamp = get_u64(&mut cur)? as i64;
    let pausec = get_u32(&mut cur)?;
    Ok(PaEncTsEnc {
        patimestamp,
        pausec,
    })
...
pub fn encode_enc_ticket_part(e: &EncTicketPart) -> Vec<u8> {
    let mut out = Vec::with_capacity(64 + e.crealm.len() + e.cname.iter().map(|s| s.len() + 2).sum::<usize>());
    out.push(ENC_TICKET_PART_MAGIC);
    put_u32(&mut out, e.flags);
    put_str(&mut out, &e.crealm);
    put_string_list(&mut out, &e.cname);
    out.extend_from_slice(&e.session_key);
    put_u64(&mut out, e.authtime as u64);
    put_u64(&mut out, e.starttime as u64);
    put_u64(&mut out, e.endtime as u64);
    put_u64(&mut out, e.renew_till as u64);
    out.extend_from_slice(e.client_uuid.as_bytes());
    out
}
...
pub fn decode_enc_ticket_part(b: &[u8]) -> Result<EncTicketPart, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != ENC_TICKET_PART_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: ENC_TICKET_PART_MAGIC,
            actual: magic,
        });
    }
    let flags = get_u32(&mut cur)?;
    let crealm = get_str(&mut cur)?;
    let cname = get_string_list(&mut cur)?;
    if cur.len() < SESSION_KEY_LEN {
        return Err(DecodeError::UnexpectedEof);
    }
    let mut session_key = [0u8; SESSION_KEY_LEN];
    session_key.copy_from_slice(&cur[..SESSION_KEY_LEN]);
    cur = &cur[SESSION_KEY_LEN..];
    let authtime = get_u64(&mut cur)? as i64;
    let starttime = get_u64(&mut cur)? as i64;
    let endtime = get_u64(&mut cur)? as i64;
    let renew_till = get_u64(&mut cur)? as i64;
    if cur.len() < 16 {
        return Err(DecodeError::UnexpectedEof);
    }
    let mut uuid_bytes = [0u8; 16];
    uuid_bytes.copy_from_slice(&cur[..16]);
...
pub fn encode_ticket(t: &Ticket) -> Vec<u8> {
    let mut out = Vec::with_capacity(64 + t.realm.len() + t.sname.iter().map(|s| s.len() + 2).sum::<usize>() + t.enc_part.len());
    out.push(TICKET_MAGIC);
    put_u32(&mut out, t.tkt_vno);
    put_str(&mut out, &t.realm);
    put_string_list(&mut out, &t.sname);
    put_u32(&mut out, t.kvno);
    put_u32(&mut out, t.etype as u32);
    put_blob(&mut out, &t.enc_part);
    out
}
...
pub fn decode_ticket(b: &[u8]) -> Result<Ticket, DecodeError> {
    ...
    if magic != TICKET_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: TICKET_MAGIC,
            actual: magic,
        });
    }
    let tkt_vno = get_u32(&mut cur)?;
    let realm = get_str(&mut cur)?;
    let sname = get_string_list(&mut cur)?;
    let kvno = get_u32(&mut cur)?;
    let etype_raw = get_u32(&mut cur)?;
    let etype = EType::from_u32(etype_raw).ok_or(DecodeError::UnknownEtype(etype_raw))?;
    let enc_part = get_blob(&mut cur)?;
    Ok(Ticket {
        tkt_vno,
        realm,
        sname,
...
pub fn encode_enc_kdc_rep_part(e: &EncKdcRepPart) -> Vec<u8> {
    let mut out = Vec::with_capacity(64 + e.crealm.len() + e.cname.iter().map(|s| s.len() + 2).sum::<usize>());
    out.push(ENC_KDC_REP_PART_MAGIC);
    out.extend_from_slice(&e.session_key);
    put_u64(&mut out, e.last_req as u64);
    put_u32(&mut out, e.nonce);
    put_u64(&mut out, e.authtime as u64);
    put_u64(&mut out, e.starttime as u64);
    put_u64(&mut out, e.endtime as u64);
    put_u64(&mut out, e.renew_till as u64);
...
pub fn decode_enc_kdc_rep_part(b: &[u8]) -> Result<EncKdcRepPart, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != ENC_KDC_REP_PART_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: ENC_KDC_REP_PART_MAGIC,
            actual: magic,
        });
    }
    if cur.len() < SESSION_KEY_LEN {
    ...
    session_key.copy_from_slice(&cur[..SESSION_KEY_LEN]);
    cur = &cur[SESSION_KEY_LEN..];
    let last_req = get_u64(&mut cur)? as i64;
    let nonce = get_u32(&mut cur)?;
    let authtime = get_u64(&mut cur)? as i64;
    let starttime = get_u64(&mut cur)? as i64;
    let endtime = get_u64(&mut cur)? as i64;
    let renew_till = get_u64(&mut cur)? as i64;
...
pub fn encode_as_req(r: &AsReq) -> Vec<u8> {
    let mut out = Vec::with_capacity(32 + r.realm.len() + r.cname.iter().map(|s| s.len() + 2).sum::<usize>());
    out.push(AS_REQ_MAGIC);
    put_u32(&mut out, r.pvno);
    put_u32(&mut out, r.msg_type);
    put_str(&mut out, &r.realm);
    put_string_list(&mut out, &r.cname);
    put_u32(&mut out, r.nonce);
    put_etype_list(&mut out, &r.etypes);
    let n_padata = u8::try_from(r.padata.len()).expect("padata count fits in u8");
    out.push(n_padata);
    for p in &r.padata {
        out.push(p.padata_type);
        put_blob(&mut out, &p.padata_value);
    }
    put_u64(&mut out, r.till as u64);
    out
}
...
pub fn decode_as_req(b: &[u8]) -> Result<AsReq, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != AS_REQ_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: AS_REQ_MAGIC,
            actual: magic,
        });
    }
    let pvno = get_u32(&mut cur)?;
    if pvno != PVNO {
        return Err(DecodeError::InvalidPvno(pvno));
    }
    let msg_type = get_u32(&mut cur)?;
    if msg_type != MSG_TYPE_AS_REQ {
        return Err(DecodeError::InvalidMsgType(msg_type));
    }
    let realm = get_str(&mut cur)?;
    let cname = get_string_list(&mut cur)?;
    let nonce = get_u32(&mut cur)?;
    let etypes = get_etype_list(&mut cur)?;
    let n_padata = get_u8(&mut cur)?;
    let mut padata = Vec::with_capacity(n_padata as usize);
    for _ in 0..n_padata {
        let padata_type = get_u8(&mut cur)?;
        let padata_value = get_blob(&mut cur)?;
        padata.push(PaData {
            padata_type,
            padata_value,
        });
    }
    let till = get_u64(&mut cur)? as i64;
    Ok(AsReq {
        pvno,
        msg_type,
        realm,
...
pub fn encode_as_rep(r: &AsRep) -> Vec<u8> {
    let mut out = Vec::with_capacity(64 + r.crealm.len() + r.cname.iter().map(|s| s.len() + 2).sum::<usize>());
    out.push(AS_REP_MAGIC);
    put_u32(&mut out, r.pvno);
    put_u32(&mut out, r.msg_type);
    put_str(&mut out, &r.crealm);
    put_string_list(&mut out, &r.cname);
    let ticket_bytes = encode_ticket(&r.ticket);
    put_blob(&mut out, &ticket_bytes);
    put_u32(&mut out, r.enc_part_etype as u32);
    put_u32(&mut out, r.enc_part_kvno);
    put_blob(&mut out, &r.enc_part);
    out
}
...
pub fn decode_as_rep(b: &[u8]) -> Result<AsRep, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != AS_REP_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: AS_REP_MAGIC,
            actual: magic,
        });
    }
    let pvno = get_u32(&mut cur)?;
    let msg_type = get_u32(&mut cur)?;
    if msg_type != MSG_TYPE_AS_REP {
        return Err(DecodeError::InvalidMsgType(msg_type));
    }
    let crealm = get_str(&mut cur)?;
    let cname = get_string_list(&mut cur)?;
    let ticket_blob = get_blob(&mut cur)?;
    let ticket = decode_ticket(&ticket_blob)?;
    let etype_raw = get_u32(&mut cur)?;
    let enc_part_etype = EType::from_u32(etype_raw).ok_or(DecodeError::UnknownEtype(etype_raw))?;
    let enc_part_kvno = get_u32(&mut cur)?;
    let enc_part = get_blob(&mut cur)?;
    Ok(AsRep {
        pvno,
        msg_type,
...
pub fn encode_authenticator(a: &Authenticator) -> Vec<u8> {
    let mut out = Vec::with_capacity(64 + a.crealm.len() + a.cname.iter().map(|s| s.len() + 2).sum::<usize>());
    out.push(AUTHENTICATOR_MAGIC);
    put_str(&mut out, &a.crealm);
    put_string_list(&mut out, &a.cname);
    match &a.subkey {
        Some(k) => {
            out.push(1);
    ...
    put_u32(&mut out, a.seq_number);
    put_u64(&mut out, a.ctime as u64);
    put_u32(&mut out, a.cusec);
    out
}
...
pub fn decode_authenticator(b: &[u8]) -> Result<Authenticator, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != AUTHENTICATOR_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: AUTHENTICATOR_MAGIC,
            actual: magic,
        });
    }
    let crealm = get_str(&mut cur)?;
    let cname = get_string_list(&mut cur)?;
    let has_subkey = get_u8(&mut cur)?;
    let subkey = if has_subkey != 0 {
        if cur.len() < SESSION_KEY_LEN {
            return Err(DecodeError::UnexpectedEof);
        }
        let mut k = [0u8; SESSION_KEY_LEN];
        k.copy_from_slice(&cur[..SESSION_KEY_LEN]);
        cur = &cur[SESSION_KEY_LEN..];
        Some(k)
    } else {
        None
    };
    let seq_number = get_u32(&mut cur)?;
    let ctime = get_u64(&mut cur)? as i64;
    let cusec = get_u32(&mut cur)?;
    Ok(Authenticator {
        crealm,
        cname,
        subkey,
...
pub fn encode_tgs_req(r: &TgsReq) -> Vec<u8> {
    let mut out = Vec::with_capacity(64 + r.realm.len() + r.sname.iter().map(|s| s.len() + 2).sum::<usize>());
    out.push(TGS_REQ_MAGIC);
    put_u32(&mut out, r.pvno);
    put_u32(&mut out, r.msg_type);
    put_str(&mut out, &r.realm);
    put_string_list(&mut out, &r.sname);
    put_u32(&mut out, r.nonce);
    put_etype_list(&mut out, &r.etypes);
    let tgt_bytes = encode_ticket(&r.tgt);
    put_blob(&mut out, &tgt_bytes);
    put_blob(&mut out, &r.authenticator_enc);
    put_u64(&mut out, r.till as u64);
    out
}
...
pub fn decode_tgs_req(b: &[u8]) -> Result<TgsReq, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != TGS_REQ_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: TGS_REQ_MAGIC,
            actual: magic,
        });
    }
    let pvno = get_u32(&mut cur)?;
    ...
    let nonce = get_u32(&mut cur)?;
    let etypes = get_etype_list(&mut cur)?;
    let tgt_blob = get_blob(&mut cur)?;
    let tgt = decode_ticket(&tgt_blob)?;
    let authenticator_enc = get_blob(&mut cur)?;
    let till = get_u64(&mut cur)? as i64;
    Ok(TgsReq {
        pvno,
        msg_type,
...
pub fn decode_tgs_rep(b: &[u8]) -> Result<TgsRep, DecodeError> {
    let mut cur = b;
    let magic = get_u8(&mut cur)?;
    if magic != TGS_REP_MAGIC {
        return Err(DecodeError::MagicMismatch {
            expected: TGS_REP_MAGIC,
            actual: magic,
        });
    }
    let pvno = get_u32(&mut cur)?;
    let msg_type = get_u32(&mut cur)?;
    if msg_type != MSG_TYPE_TGS_REP {
        return Err(DecodeError::InvalidMsgType(msg_type));
    }
    let crealm = get_str(&mut cur)?;
    let cname = get_string_list(&mut cur)?;
...
mod tests {
    ...
    #[tokio::test]
    async fn as_req_handler_rejects_wrong_magic() {
        let store = InMemoryPrincipalStore::new();
        let krbtgt_key = make_krbtgt_key();
        let err = handle_as_req(&store, &krbtgt_key, &[AS_REP_MAGIC])
            .await
